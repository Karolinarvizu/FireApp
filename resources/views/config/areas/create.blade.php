...
<div class="container" style="max-width:500px">
    <h1><i class="fas fa-plus-circle me-2" style="color:#cc2200;font-size:1rem"></i>Agregar área</h1>
    <div style="background:#1e1e1e;border:1px solid #2a2a2a;border-radius:10px;padding:24px;">
        <form action="{{ route('config.areas.store') }}" method="POST">
            @csrf
            <div class="mb-3">
                <label class="form-label" for="name">Nombre del área</label>
                <input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror"
                    value="{{ old('name') }}" placeholder="Ej: Baño, Cocina, Dormitorio" required>
